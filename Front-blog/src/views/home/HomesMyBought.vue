...
const traceEventMap = {
  ...
  GOODS_ORDER_LOCKED: '商品库存锁定',
  GOODS_ORDER_PAID: '商品交易支付同步',
  GOODS_ORDER_SENT: '商品交易发货同步',
  GOODS_OWNERSHIP_TRANSFERRED: '商品权属变更',
  GOODS_ORDER_RESTOCKED: '商品库存恢复'
};
...
const getTraceAlertType = (trace) => {
  if (!trace?.traceId) return 'info';
  return trace.verified ? 'success' : 'error';
};

const getTraceStatusText = (trace) => {
  if (!trace?.traceId) return '未上链';
  return trace.verified ? '校验通过' : '校验异常';
};
